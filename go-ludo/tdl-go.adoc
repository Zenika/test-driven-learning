...
Je me suis demandé comment commencer : faire des tutoriels, souvent cela se résume à des copier-coller et finit par une connaissance très partielle.
...
Il n'y a donc pas d'assertions de base dans Go mais juste un structure permettant d'enregistrer les erreurs.
...
Je ne connais pas encore le langage mais je sais comment écrire un test !
...
Super autocomplétion des bibliothèques, le style de l'IDE est moche mais bon si va bien c'est OK.
Mais *fail* car le support du clavier est destiné uniquement au clavier QWERTY.

Donc c'est reparti pour tester un nouvel IDE.

J'avais déjà installé https://atom.io[Atom] donc je regarde les plugins disponible et surprise :
...
Après quelques lectures de la documentation et de quelques samples, je comprends que dans Go est principalement un langage fonctionnel où les fonctions sont simplement exposée par le package.
...
Pour cela, il me faut déclarer des variables, je regarde les sources exemples du GitHub officiel.
...
Il ne me reste plus qu'à traiter le fait de traiter une séquence de 1 à 100.
...
Je sûrement faire un tour de Go : https://tour.golang.org/welcome/
